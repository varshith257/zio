...
class SmartAssertMacros(val c: blackbox.Context) {
  ...
  // Detect if it's a `.view` or `.mapValues` call and force evaluation
  private def forceViewEvaluation(tree: c.Tree): c.Tree = {
    tree match {
      case q"$_.view.mapValues(..$args).toMap" =>
        // Match and evaluate to a strict collection
        q"${tree}.toMap"
      case _ => tree
    }
  }
  ...
  def assertOne_impl(expr: Expr[Boolean]): c.Tree = {
    ...
    val processedTree = forceViewEvaluation(tree)
