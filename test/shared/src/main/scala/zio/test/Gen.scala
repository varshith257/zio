...
object Gen extends GenZIO with FunctionVariants with TimeVariants {
   ...
   */
  def scopedRandom[R1 <: Any, A](gen: Gen[Random, A])(implicit trace: Trace): Gen[Any, A] =
    Gen.fromZIO(
      ZIO.randomWith(random =>
        gen.sample
          .map(_.value)                             // Extract the value from the Sample
          .provideEnvironment(ZEnvironment(random)) // Provide fresh Random environment
          .runHead                                  // Run the ZStream and take the first sample
          .flatMap {
            case Some(value) => ZIO.succeed(value) // If a value is present, return it
            case None        => ZIO.die(new NoSuchElementException("No value generated by the random generator"))
          }
      )
    )
